...
export async function createTeam(gameId: number, teamName: string): Promise<Team | undefined> {
    try {
        const config = getHeaders();
        const response = await axiosInstance.post(`/games/${gameId}/teams`, {"team_name": teamName}, config);
        if(response.status === 200) {
