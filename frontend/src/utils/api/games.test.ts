...
describe('getGames', () => {
    it("should return a list of games on success", async () => {
        ...
        expect(api.axiosInstance.get).toBeCalledWith("/games", {headers: {"Authorization": `bearer abc`}});
        expect(games).toEqual(mockResponse.data)
    });

    it('should return undefined when the API call fails', async () => {
        api.axiosInstance.get = jest.fn().mockReturnValue({ status:400, data: { error: "invalid login credentials" }});
        const games = await getGames();
